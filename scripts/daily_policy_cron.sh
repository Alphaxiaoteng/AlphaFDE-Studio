#!/bin/bash
# ==============================================================================
# 云谷中心 · 每日政策雷达自动搜集与三层漏斗有效性清洗 Cron 任务脚本
# 触发时机：每日早间 08:00
...
echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] 触发每日政策雷达自动清洗与更新任务..." >> "$LOG"
...
if [ $EXIT_CODE -eq 0 ]; then
  echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] 每日政策雷达清洗成功完成 (Exit Code 0)。" >> "$LOG"
  echo "✓ 每日政策清洗完成，已同步落盘 data.json 并记录审计日志。"
else
  echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] 每日政策雷达清洗异常，退出码: $EXIT_CODE" >> "$LOG"
