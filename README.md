...
3 人企服编制下，把政策辅导做成可核验流水线：**企服雷达初筛 → 三态判定（符合/排除/待核验）→ 专员放行 → 微信草稿触达 → SOP 留痕**。  
...
| **企服雷达** | 政府政策 / 公开活动；`urgency` + `days_left` 紧急窗口置顶；条款 `citation` |
...
```bash
# Skills 喂给 Agent（部署版；创空间 SPA 页无法直接 curl skills.md）
curl -fsSL -A "Mozilla/5.0" https://cp1024-alphafde-studio.ms.show/skills.md
```

- UI 直连：https://cp1024-alphafde-studio.ms.show/
...
python3 server.py
# 本地调试 http://127.0.0.1:8766（仅开发；安装命令默认用上方部署 URL）
```
