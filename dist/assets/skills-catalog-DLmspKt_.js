...
When planning, show a concise brief and shot list. When executing, report generated clips, timeline structure, and any shots that need user review.`,t=`# Cinematic AI Workflow and Style Bible
...
- [ ] Composed frames and playback confirm the result is ready for user review or export.`,n=`---
name: asset-import
...
- Do not batch many shorts blindly. If the style is not established, create or preview the first strong clip before repeating it.`,l=`# Short-Form Candidate Selection
...
- The final edit feels clearer, not busier.`,d=`---
...
- For multiple requested reels or short videos, create distinct angles instead of near-duplicate edits from the same footage.`,f=`# Short-Form Candidate Selection
...
For planning, show angle, target viewer, hook, script beats, visual plan, captions, CTA, and assumptions. For execution, also report what was added to the timeline.`,v=`# Evidence-Grounded Short-Video Ad Framework
...
`,b=`# Generation capabilities (as wired)
...
`,x=`# Official generation API documentation
...
\`\`\`
`,O=`# Shader Effect & Transition Design Principles
...
- Prefer current OpenChatCut/media tools over hardcoded local commands. Use external scripts only when they are available and clearly help.`,j=`---
...
- \`references/field-testing-notes.md\`: known failure modes.`,R=`---
name: voice
description: |
...
\`\`\`
`,U=/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/;function W(e){let t=e.match(U);if(!t)return{name:``,description:``,body:e};let[,n,r]=t;return{name:G(n),description:K(n),body:r}}function G(e){let t=e.match(/^name:\s*(.*)$/m);return t?q(t[1].trim()):``}function K(e){let t=e.split(/\r?\n/),n=t.findIndex(e=>/^description:/.test(e));if(n<0)return``;let r=t[n].replace(/^description:\s*/,``);if(r.trim()===``||/^[|>][-+]?\s*$/.test(r.trim())){let e=[];for(let r=n+1;r<t.length;r+=1){let n=t[r];if(n.trim()===``){e.push(``);continue}if(!/^\s/.test(n))break;e.push(n.replace(/^\s+/,``))}return e.join(` `).replace(/\s+/g,` `).trim()}return q(r.trim())}function q(e){return e.length>=2&&(e.startsWith(`"`)&&e.endsWith(`"`)||e.startsWith(`'`)&&e.endsWith(`'`))?e.slice(1,-1):e}var J=Object.assign({"./ai-cinematic-short-film/SKILL.md":e,"./ai-cinematic-short-film/references/cinematic-ai-workflow.md":t,"./asset-import/SKILL.md":n,"./create-motion-graphics/SKILL.md":r,"./create-motion-graphics/references/canvas-pipeline-rules.md":i,"./explainer-video/SKILL.md":a,"./explainer-video/references/explainer-animation.md":o,"./export/SKILL.md":s,"./image-gen/SKILL.md":ee,"./image-gen/references/gpt-image-2.md":te,"./image-gen/references/image-01.md":ne,"./image-gen/references/nano-banana.md":re,"./known-errors/SKILL.md":ie,"./long-video-to-shorts/SKILL.md":c,"./long-video-to-shorts/references/short-form-selection.md":l,"./motion-graphic-placement/SKILL.md":u,"./multi-clips-to-reels/SKILL.md":d,"./multi-clips-to-reels/references/short-form-selection.md":f,"./music/SKILL.md":p,"./music/references/minimax.md":m,"./music/references/mureka.md":h,"./openchatcut-plugin-basics/SKILL.md":g,"./product-ad-video-script/SKILL.md":_,"./product-ad-video-script/references/ad-script-framework.md":v,"./product-help/SKILL.md":y,"./product-help/references/generation-capabilities.md":b,"./product-help/references/generation-official-docs.md":x,"./product-help/references/providers-and-keys.md":S,"./product-help/references/ui-and-features.md":C,"./shader-gen/SKILL.md":w,"./shader-gen/examples/cube-rotate.md":T,"./shader-gen/examples/door-open.md":E,"./shader-gen/examples/page-curl.md":D,"./shader-gen/references/design-principles.md":O,"./shader-gen/references/property-changes.md":k,"./storyboard-shot-breakdown/SKILL.md":A,"./talking-head-guide/SKILL.md":j,"./transcription/SKILL.md":ae,"./verification/SKILL.md":M,"./video-gen/SKILL.md":N,"./video-gen/references/hailuo.md":P,"./video-gen/references/kling.md":F,"./video-gen/references/seedance2.md":I,"./video-thumbnail-generator/SKILL.md":L,"./voice/SKILL.md":R,"./voice/references/minimax-tts.md":z,"./voice/references/video-sync.md":B,"./voice/references/voices.md":V,"./widget-forms/SKILL.md":H}),Y=e=>e.replace(/^\.\//,``).split(`/`)[0],X=Object.entries(J).filter(([e])=>e.endsWith(`/SKILL.md`)).map(([e,t])=>{let n=Y(e);return{slug:n,files:Object.keys(J).filter(e=>Y(e)===n&&!e.endsWith(`/SKILL.md`)).map(e=>e.replace(`./${n}/`,``)).sort(),...W(t)}}).sort((e,t)=>e.slug.localeCompare(t.slug));function Z(e){return X.find(t=>t.slug===e)}function oe(e,t){return t?J[`./${e}/${t.replace(/^\.\//,``)}`]:Z(e)?.body}var se=[``,`# Skill library (load_skill on demand · ${X.length} OpenChatCut SKILL.md files)`,`Each entry below describes when a skill applies. When a task matches one, call load_skill(name=…) to retrieve its full workflow before acting. For deeper material, pass file= (for example, "references/voices.md"). Load only relevant skills, not the entire library.`,`When a skill needs scripts, ffmpeg, node, or python, use run_code in the isolated sandbox: write files, run a command, then read outputs. The sandbox cannot access the timeline; use local editor tools to place resulting assets. For real media, pass a local /media/… or public https:// URL in files so the sandbox can fetch it for ffprobe/ffmpeg. Public URLs may also be passed directly to ffprobe.`,"The custom sandbox image includes ffmpeg. If an environment lacks it, install it first with: `which ffmpeg || (sudo apt-get update -qq && sudo apt-get install -y -qq ffmpeg)`.",...X.map(e=>`- **${e.slug}** — ${e.description}`)].join(`
`),Q=[{id:`11111111-1240-4000-8000-000000000004`,slug:`long-video-to-shorts`,name:`Long Video to Shorts`,nameZh:`长视频转短视频`,summary:`把一条长播客、访谈、课程或直播剪成适合社媒发布的短视频和高光。`,scenarios:[`long-video-to-shorts`,`reels`,`tiktok`,`shorts`,`social-clips`,`highlight-reel`]},{id:`11111111-1240-4000-8000-000000000012`,slug:`multi-clips-to-reels`,name:`Multi Clips to Reels`,nameZh:`多素材剪 Reels`,summary:`把产品、活动、旅行或游戏素材剪成适合社媒发布的 Reels。`,scenarios:[`multi-clips-to-reels`,`multi-clips-to-shorts`,`raw-clips`,`reels`,`tiktok`,`shorts`]},{id:`11111111-1240-4000-8000-000000000005`,slug:`ai-cinematic-short-film`,name:`AI Cinematic Short Film`,nameZh:`AI 电影感短片`,summary:`规划并制作 AI 电影感短片，覆盖故事、镜头、提示词、连续性和最终检查。`,scenarios:[`ai-film`,`cinematic`,`seedance`,`story`,`short-film`,`video-generation`]},{id:`11111111-1240-4000-8000-000000000006`,slug:`product-ad-video-script`,name:`Product Ad Video Script`,nameZh:`产品广告脚本`,summary:`把产品或页面转成广告角度、开头钩子、分镜、字幕、CTA 和视觉方向。`,scenarios:[`ad`,`e-commerce`,`landing-page`,`marketing`,`product`,`script`]},{id:`11111111-1240-4000-8000-000000000011`,slug:`explainer-video`,name:`Explainer Video`,nameZh:`解说视频制作`,summary:`把主题、脚本、配音、产品逻辑或数据做成完整解说视频。`,scenarios:[`concept`,`course`,`data`,`education`,`explainer`,`explainer-video`]},{id:`11111111-1240-4000-8000-000000000008`,slug:`motion-graphic-placement`,name:`Motion Graphic Placement`,nameZh:`动效点缀指南`,summary:`在合适时机添加动效，强化表达且不遮挡内容。`,scenarios:[`creator-video`,`interview`,`lecture`,`motion-graphic-placement`,`motion-graphics`,`podcast`]},{id:`11111111-1240-4000-8000-000000000009`,slug:`storyboard-shot-breakdown`,name:`Storyboard Shot Breakdown`,nameZh:`拉片分镜图`,summary:`逐镜拆解镜头语言，并生成分镜参考图。`,scenarios:[`cinematography`,`director-logic`,`film-analysis`,`shot-analysis`,`shot-breakdown`,`storyboard`]},{id:`11111111-1240-4000-8000-000000000010`,slug:`video-thumbnail-generator`,name:`Video Thumbnail Generator`,nameZh:`视频封面生成`,summary:`基于视频内容和真实画面生成适合平台的封面图。`,scenarios:[`bilibili-cover`,`cover-image`,`poster`,`shorts-cover`,`thumbnail`,`video-cover`]}].flatMap(e=>{let t=Z(e.slug);if(!t)throw Error(`Creative skill metadata references missing SKILL.md: ${e.slug}`);return[{...e,description:t.description,body:t.body,files:t.files,source:`builtin`}]}),$=[];function ce(e){$=e}function le(){return[...Q,...$]}var ue=e=>e?Q.find(t=>t.id===e)??$.find(t=>t.id===e):void 0;export{X as a,W as c,ce as i,le as n,se as o,ue as r,oe as s,Q as t};